...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    const { price, quantity = 1, metadata = {}, isForMembership = false, cancelRedirectPath } = req.body;
    ...
    try {
      ...
      const stripeSession = await stripe.checkout.sessions.create({
        ...
        cancel_url: cancelRedirectPath ? getURL() + cancelRedirectPath : `${getURL()}/${isForMembership ? 'membership' : 'signup'}`,
      });
