  ...
  if (req.method === 'GET') {
    ...
    if (!searchString)
      result = await prisma.$queryRaw<PartialAttendee[]>`
        SELECT  e.name as eventName, c.eventId as eventId, c.userId as userId, c.emailOptIn as "emailOptIn", u.name, u.email, c.discoveredFrom, c.id as "checkinId", c.createdDate, CASE WHEN s.lastPaymentDate >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR) THEN true ELSE false END as "isMember"
