...
describe('Checkin Component', () => {
  ...
  it('deletes a check-in when requested', async () => {
    // Mock the parsedGet response for a user
    (parsedGet as jest.Mock).mockImplementation(async url => {
      if (url.includes('/api/events/1/checkin')) {
        return {
          isFound: false,
    ...
    // Mock axios.delete for the delete operation
    (axios.delete as jest.Mock).mockResolvedValueOnce({});

    // Render the component with a user checked in
    renderWithQueryClient(<Checkin event={mockEvent} />);
    ...
    fireEvent.change(firstNameInput, { target: { value: 'Delete' } });
    fireEvent.change(lastNameInput, { target: { value: 'User' } });
